...
import { useGetWeeklyPopulerArtistQuery } from "@/redux/features/artist/artist.api";
...
const WeeklyTrendingArtists = () => {
  const { data, isLoading } = useGetWeeklyPopulerArtistQuery(undefined);
  const artists = data?.data || [];
  return (
    <Card>
      <SectionHeader title="Populer Thi Week" subtitle={`Trending Artists`} />
        ...
        {isLoading ? (
          ...
          artists.map((a) => (
            <SwiperSlide key={a._id}>
              <ArtistComonCard artist={{ ...a, popular: true }} />
            </SwiperSlide>
          ))
        )}
...
export default WeeklyTrendingArtists;
