  ...
  return (
    <section className="mb-[60px] relative w-full h-full py-[24px] px-[20px]">
      <div className="mb-[20px]">
        <h2 className="mb-[8px]">Instant Playlists</h2>
      ...
      >
        {instantPlaylists.map((playlist) => (
          <SwiperSlide key={playlist.id}>
            <Link href={`/instant-playlist/${playlist.id}`}>
              ...
              <div>
                <p className="font-[500]">{playlist.name}</p>
                <p className="text-muted mb-[4px]">{playlist.designation}</p>
                <div className="flex items-center gap-[4px] font-[500] mb-[4px]">
                  <p className="flex items-center gap-[4px]">
                    <StarIcon className="h-[14px] w-[14px]" /> {playlist.rating}
                  </p>
                  <p>({playlist.reviews})</p>
                </div>
                <div className="flex items-center gap-[4px] font-[500]">
                  <p className={cn("text-light", playlist.oldPrice && "text-greeniest")}>
                    ${playlist.price} {!playlist.oldPrice && "+"}
                  </p>
                  {playlist.oldPrice && (
                    <p className="text-muted line-through">${playlist.oldPrice}+</p>
                  )}
                </div>
              </div>
            </Link>
...
export default InstantPlaylists;
